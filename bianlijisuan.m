function  [mubiao,fengxian,xxx]=bianlijisuan(TTR,ES_pingheng,duan_ES,MSOC,pv_state,state_GP,...
    QL,QG,qie_L,qie_G,fuheshu,PV,PV00,es,es1,es2,es3,shu_NP,state_ES,ES,xxx)
tongji1=shu_NP*ones(1,4);
if  TTR==1
   tongji1(2:4)=1;
elseif TTR==2
   tongji1(3:4)=1;
...
jishu1=0;
  tongji=1;
  gailvJI=ones(prod(tongji1),1);
             E_f0=zeros(prod(tongji1),1);
             p_h10=zeros(prod(tongji1),1);
             p_h20=zeros(prod(tongji1),1);
             p_h30=zeros(prod(tongji1),1);
             p_h40=zeros(prod(tongji1),1);
             p_g10=zeros(prod(tongji1),1);
               for t1=1:tongji1(1) %%TTR=1的初始时刻
               for t2=1:tongji1(2)%%%TTR=2的初始时刻
               for t3=1:tongji1(3)%%%TTR=3的初始时刻
               for t4=1:tongji1(4)%%%%TTR=4的初始时刻
              paixu=10^10*ones(1,TTR+1);
            ...
            soc=soc0;
             ...
             for tt=1:TTR  %%一小时一小时判断                
                  ...
                  if state_GP(i,T)>1
                      gailvJI(tongji)=gailvJI(tongji)*pv_state(i,state_GP(i,T),tt);
                  else
                      gailvJI(tongji)=0;
                  end
...
%              xxx=[xxx;paixu];
             jishu=jishu+gailvJI(tongji);
             if paixu(1)~=0
                 jishu1=jishu1+gailvJI(tongji);
             end
             E_f0(tongji)=sum(E_f);
              p_h10(tongji)=(1-prod(ones(1,TTR)-p_h1));
             p_h20(tongji)=(1-prod(ones(1,TTR)-p_h2));
             if paixu(1)~=0
             p_h30(tongji)=(1-prod(ones(1,TTR)-p_h3));
             end
             p_h40(tongji)=(1-prod(ones(1,TTR)-p_h4));
             p_g10(tongji)=(1-prod(ones(1,TTR)-p_g1));
             end
             tongji=tongji+1;
               end
               end
       ...
       mubiao=0;
       fengxian=zeros(5,1);
               for i=1:prod(tongji1) %%TTR=1的初始时刻
                mubiao=mubiao+E_f0(i)*gailvJI(i)/jishu;
                fengxian(1)=fengxian(1)+p_h10(i)*gailvJI(i)/jishu;
                fengxian(2)=fengxian(2)+p_h20(i)*gailvJI(i)/jishu;
%                 fengxian(3)=fengxian(3)+p_h30(i)*gailvJI(i)/jishu1;
                fengxian(4)=fengxian(4)+p_h40(i)*gailvJI(i)/jishu;
                fengxian(5)=fengxian(5)+p_g10(i)*gailvJI(i)/jishu;
               end
